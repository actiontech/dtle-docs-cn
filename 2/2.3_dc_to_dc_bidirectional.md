...
```
docker exec -u root -it dtle-dc1 vi /dtle/etc/nomad/nomad.hcl
...
docker exec -u root -it dtle-dc1 rm -rf /dtle/var/lib/nomad
docker restart dtle-nomad
```

配置`/dtle/etc/nomad/nomad.hcl`修改的内容如下: 
...
```
docker exec -u root -it dtle-dc2 vi /dtle/etc/nomad/nomad.hcl
...
