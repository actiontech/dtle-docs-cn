...
- 从3.22.07.0开始，源端(src)固定 **只填写** `GetConfigFrom = "consul"`
- 其他dtle参数转移到目标端(dest)。
  - 特别地 `ConnectionConfig` 分别改为 `SrcConnectionConfig`和`DestConnectionConfig`
- 原有配置方式仍被兼容。

| 参数名                   | 必填? | 类型       | 默认值         | 说明                                                                                                                                |
|-----------------------|-----|----------|-------------|-----------------------------------------------------------------------------------------------------------------------------------|
| Gtid                  | 否   | String   | "" | MySQL的GTID集合(区间), 可取值: <br/> 1. 空（默认值）, 执行 <全量+增量> 复制任务 <br/> 2. 已复制的GTID集合(不是点位), 将从未复制的GTID开始增量复制                               |
...
| ReplicateDoDb         | 否   | Object数组 | -           | 要复制的库表。如为空`[]`, 则复制整个数据库实例. 可填写多元素. 元素内容见下方说明                                                                                     |
| ReplicateIgnoreDb     | 否   | Object数组 | -           | 指定要忽略的库表，优先级高于ReplicateDoDb。如为空`[]`则不忽略任何库表. 可填写多元素. 元素内容见下方说明                                                                    |
| SrcConnectionConfig   | 否   | Object   | -           | MySQL源端信息, 见下方 ConnectionConfig 说明。和OracleConfig二选一填写                                                                             |
| DestConnectionConfig  | 否   | Object   | -           | MySQL目标端信息, 见下方 ConnectionConfig 说明。和KafkaConfig二选一填写。                                                                            |
| OracleConfig          | 否   | Object   | -           | Oracle源端信息, 见下方 OracleConfig 说明。和ConnectionConfig二选一填写。                                                                           |
| KafkaConfig           | 否   | Object   | -           | Kafka目标端信息, 见下方 KafkaConfig 说明。和DestConnectionConfig二选一填写。                                                                        |
