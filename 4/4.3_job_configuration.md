...
| DumpEntryLimit        | 否   | Int      | 源端        | 67108864 (64M) | 复制时, 读取后分块发送的分块大小。空闲内存较小时需适当调小。适用于大全量/增量大事务                                                                                       |
...
| SlaveNetWriteTimeout  | 否   | Int      | 源端        | 28800 (8小时)    | 调整MySQL slave线程的超时时间。MySQL默认值为60，太短可能导致断连。太长则会导致异常连接回收不及时。                                                                        |
| BulkInsert1           | 否   | Int      | 目标端       | 4              | 批量插入第一级数量。见[性能调优](4.3.1_tuning.md)                                                                                                |
| BulkInsert2           | 否   | Int      | 目标端       | 8              | 批量插入第二级数量。                                                                                                                        |
| BulkInsert3           | 否   | Int      | 目标端       | 128            | 批量插入第三级数量。                                                                                                                        |
| SetGtidNext           | 否   | Bool     | 目标端       | false          | 目标端执行事务前执行`set gtid_next = ...`, 使源端目标端MySQL事务gtid相同。可用以避免循环复制。需要 `REPLICATION_APPLIER` (MySQL 8.0)或 `SUPER` 权限                   |
...
| Table.ColumnMapTo   | 否           | String数组        | 源端            | -             | 列映射（暂不支持正则表达式）。见[demo](../3/3.8_dtle_mapping.md)        |
